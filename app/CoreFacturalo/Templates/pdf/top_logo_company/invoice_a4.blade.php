...
@endif
<table class="full-width" style="border-bottom: 1px solid #ccc">
    <tr>
        @if($company->logo)
            <td width="60%">
        ...
        @else
            <td width="110%">
                {{--<img src="{{ asset('logo/logo.jpg') }}" class="text-right" style="max-width: 200px">--}}
            </td>
        @endif
        <td width="20%" class="pl-3" style="text-align: right;">
    ...
    <tr>
        <td class="align-top pt-20">
            <table class="full-width">
                ...
                <tr>
                    @if ($document->detraction)
                        <td>MÉTODO DE PAGO: {{ $detractionType->getPaymentMethodTypeDescription($document->detraction->payment_method_id ) }}</td>
                    @endif
                </tr>
                <tr>
                ...
                <tr>
                    @if ($document->detraction)
                        <td width="120px">P. DETRACCIÓN: {{ $document->detraction->percentage}}%</td>
                    @endif
                </tr>
                @if ($customer->address !== '')
                    <tr>
                ...
                @if ($document->detraction)
                    <tr>
                        <td width="120px">MONTO DETRACCIÓN: {{ $document->currency_type->symbol }} {{ $document->detraction->amount}}</td>
                    </tr>
                @endif
                @if ($document->detraction)
                    @if($document->detraction->pay_constancy)
                        <tr>
                            <td width="120px">CONSTANCIA DE PAGO: {{ $document->detraction->pay_constancy}}</td>
                        </tr>
                    @endif
                @endif
            </table>
        ...
        <td class="pt-20">
            <table class="full-width" style="text-align: right;">
                <tr>
                    <td>
                        <strong>
                            <h3 class="color-blue">{{ $document->document_type->description }}</h3>
                            <h3 class="color-blue">{{ $document_number }}</h3>
                        </strong>
                    </td>
                </tr>
                <tr>
                ...
                @if ($document->detraction)
                    <tr>
                        <td>N. CTA DETRACCIONES: {{ $document->detraction->bank_account}}</td>
                    </tr>
                @endif
                @if($invoice)
                    <tr>
...
<table class="full-width mt-10 mb-10" style="border-spacing: 3px; border-collapse: separate;">
    <thead class="">
        <tr class="bg-grey">
            <th class="text-center py-3 bg-blue color-white" width="8%">CANT.</th>
            <th class="text-center py-3" width="8%">UNIDAD</th>
            <th class="text-center py-3">DESCRIPCIÓN</th>
            <th class="text-center py-3" width="8%">LOTE</th>
            <th class="text-center py-3" width="8%">SERIE</th>
            <th class="text-right py-3" width="12%">P.UNIT</th>
            <th class="text-right py-3" width="8%">DSCTO.</th>
            <th class="text-right py-3 bg-blue color-white" width="12%">TOTAL</th>
        </tr>
    </thead>
    <tbody class="">
    @foreach($document->items as $row)
        <tr>
            <td class="text-center align-top bg-blue color-white">
            ...
            <td class="text-right align-top bg-grey">{{ $row->item->unit_type_id }}</td>
            <td class="text-right align-top bg-grey">
            ...
            <td class="text-center align-top bg-grey">
            ...
            <td class="text-right align-top bg-grey">{{ number_format($row->unit_price, 2) }}</td>
            <td class="text-right align-top bg-grey">
                @if($row->discounts)
                    @php
            ...
            <td class="text-right align-top bg-blue color-white">{{ number_format($row->total, 2) }}</td>
        </tr>
    @endforeach
