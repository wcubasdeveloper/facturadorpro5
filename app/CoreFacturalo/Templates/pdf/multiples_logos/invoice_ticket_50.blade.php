...
@if ($establishment->logo ?? false)
    <div class="text-center company_logo_box_sm pt-5 desc-9">
        <img src="data:{{mime_content_type(public_path($establishment->logo))}};base64, {{base64_encode(file_get_contents(public_path($establishment->logo)))}}" alt="{{$company->name}}" class="company_logo_sm content">
    </div>
@else
    @if($company->logo)
        <div class="text-center company_logo_box_sm pt-5 desc-9">
